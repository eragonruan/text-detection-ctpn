...
def anchor_target_layer(rpn_cls_score, gt_boxes, im_info, _feat_stride=[16, ], anchor_scales=[16, ],image_name=None):
    ...
    logger.debug("argmax_overlaps存着gt索引:%r", argmax_overlaps.shape)
    ...
    logger.debug("每行(anchor)里面，最大的IoU值：max_overlaps:%r", max_overlaps)
    ...
    logger.debug("每列（GT）里面，这个GT对应IoU最大的anchor的行号(gt_argmax_overlaps):%r", gt_argmax_overlaps.shape)
    # [行号,行号,行号,行号...] 一共列数个
    ...
    if FLAGS.debug_mode:
        ...
        logger.debug("[调试画图] 画出所有IoU大于0.7的anchors，粉色")
        for anchor in __debug_iou_more_0_7_anchors:
             draw.rectangle(anchor.tolist(), outline=PINK)

        logger.debug("[调试画图] 画出所有和GT相交最大的anchor，橘色")
        for anchor in __debug_iou_max_with_gt_anchors:
             draw.rectangle(anchor.tolist(), outline=ORANGE)

        logger.debug("[调试画图] 画出所有正例，红色")
        for anchor in positive_anchors:
             draw.rectangle(anchor.tolist(), outline=RED)

        logger.debug("[调试画图] 画出所有负例，灰色")
        for anchor in negative_anchors:
             draw.rectangle(anchor.tolist(), outline=GRAY)

        logger.debug("[调试画图] 画出所有的GT，绿色")
        for gt in gt_boxes:
